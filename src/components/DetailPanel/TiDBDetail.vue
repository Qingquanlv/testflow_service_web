<template>
    <div :data-clazz="model.clazz">
        <div class="panelTitle">{{i18n['tidb']}}</div>
        <div class="panelBody">
...
<script>
  ...
  export default {
    ...
    data() {
      return {
          env: (this.model.exec_params && this.model.exec_params.env) || '',
          initsql: (this.model.exec_params && this.model.exec_params.sql_str)
      }
    },
    watch:{
      env(nv,ov){
        this.onChange('exec_params', {env: nv, sql_str : (this.model.exec_params && this.model.exec_params.sql_str) || ''})
      }
    },
    methods: {
      sqlChange(val){
